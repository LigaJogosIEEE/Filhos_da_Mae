...
function LifeForm:getLives()
	return self.lives
end

function LifeForm:getLife()
	return self.life.have, self.life.total
end

function LifeForm:isDead()
	return self.life.have <= 0
end

function LifeForm:increaseLives(amount)
	self.lives = self.lives + amount
end

function LifeForm:increaseLife(amount)
	self.life.total = self.life.total + amount
	self.life.have = self.life.have + amount
end

function LifeForm:increaseSalary(amount)
...
function LifeForm:takeDamage(amountDamage)
	...
	if self.life.have <= 0 and self.lives > 0 then
		self.life.have = self.life.total
